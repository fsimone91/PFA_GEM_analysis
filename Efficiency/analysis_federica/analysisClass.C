...
void outTreeInit(TTree *tree_, int &prop_region, int &prop_layer, int &prop_chamber, int &prop_etaP, float &prop_pt, float &prop_eta, float &prop_errR, float &prop_errPhi, int &matched, int &bad_chamber){
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   int matched = 0, bad_chamber = 0;
   outTreeInit(outTree, prop_region, prop_layer, prop_chamber, prop_etaP, prop_pt, prop_eta, prop_errR, prop_errPhi, matched, bad_chamber);
...
//                  if( !(abs(genParticle_PdgId->at(g)) == 13) ) continue; //gen particle is not a muon 
      ...
      for(int j=0; j<mu_propagated_pt->size(); j++){
         bool isZ = false;
          ...
          if( mu_propagated_pt->at(j) < 20 ) continue; //cut at 20 GeV
          if( isMC && (abs(mu_propagated_eta->at(j))<1.55 || abs(mu_propagated_eta->at(j)))>2.18 ) continue;

          //requirements on propagation error
          //if(mu_propagatedGlb_errR->at(j) > 1.0) continue;
          //if(mu_propagatedGlb_errPhi->at(j) > 0.2E-3) continue;

          //requirements on fiducial region
          if( mu_propagatedGlb_r->at(j) - mu_propagated_EtaPartition_rMin->at(j) < 1) continue;
          if(-mu_propagatedGlb_r->at(j) + mu_propagated_EtaPartition_rMax->at(j) < 1) continue;

          if(mu_propagated_chamber->at(j) != 19){
              if( mu_propagatedGlb_phi->at(j) - mu_propagated_EtaPartition_phiMin->at(j) < 0.005) continue;
              if(-mu_propagatedGlb_phi->at(j) + mu_propagated_EtaPartition_phiMax->at(j) < 0.005) continue;
          }else{
