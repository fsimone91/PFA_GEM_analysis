...
void plot_eff_pt()
{
    ...
    //TString inputfile = "/lustre/cms/store/user/fsimone/GEM_PFA/Analysis/20210518_1733/AnalysedTree_MC_Zmumu_efficiency_merged.root"; ///lustre/cms/store/user/fsimone/GEM_PFA/Analysis/20210518_1143/AnalysedTree_data_341343_efficiency_merged_4.root";
    ...
    TString binning = "(10, 20, 80)";
    TString den_sel = "prop_pt>20 && prop_errR < 0.6 "; //&& prop_errPhi < 0.16E-3" ;
    TString num_sel = den_sel + "&& matched";
    TString den_name = "hden";
    TString num_name = "hnum";
    
    t->Draw("prop_pt>>"+num_name+binning, num_sel);
    hnum = (TH1D *)gDirectory->Get(num_name);
    t->Draw("prop_pt>>"+den_name+binning, den_sel);
    ...
    hden->GetXaxis()->SetTitle("p_{T} (GeV)");
    ...
    h_x_ratio->GetXaxis()->SetTitle("p_{T} (GeV)");
    ...
    cout<<"run"+run<<endl;
    c1->SaveAs("plots/Efficiency/Inclusive/Eff_pt_run"+run+".png");
