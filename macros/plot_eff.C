...
void plot_eff()
{
    ...
    for(int e=0; e<2; e++){
        ...
        for(int l=0; l<2; l++){
            //loop on chambers
            for(int c=0; c<36; c++){
		TString den_sel = "prop_region == "+endcap_sel[e]+" && prop_layer == "+std::to_string(l+1)+" && prop_chamber == "+std::to_string(c+1);
                TString num_sel = den_sel + "&& matched";
                TString den_name = "hden["+std::to_string(e)+"]["+std::to_string(l)+"]["+std::to_string(c)+"]";
                TString num_name = "hnum["+std::to_string(e)+"]["+std::to_string(l)+"]["+std::to_string(c)+"]";

                t->Draw("prop_etaP>>"+num_name+binning, num_sel);
                hnum[e][l][c] = (TH1D *)gDirectory->Get(num_name);
                t->Draw("prop_etaP>>"+den_name+binning, den_sel);
                hden[e][l][c] = (TH1D *)gDirectory->Get(den_name);
                ...
                hden[e][l][c]->SetTitle("run"+run+", "+chname);
                hden[e][l][c]->GetXaxis()->SetTitle("GE1/1 eta partition");
                hden[e][l][c]->GetYaxis()->SetTitle("recHits");
                hden[e][l][c]->Draw("hist");
                hnum[e][l][c]->SetLineColor(kRed);
                hnum[e][l][c]->Draw("hist same");
                ...
                TH1D *h_x_ratio = (TH1D*)hnum[e][l][c]->Clone("h_x_ratio");
                h_x_ratio->Sumw2(); 
                h_x_ratio->Divide(hden[e][l][c]);
                ...
                c1->SaveAs("plots/Efficiency/Eff_etaP_run"+run+"_"+chname+".png");
            }
        }
    }
}
